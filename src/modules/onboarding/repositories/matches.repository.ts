    ...
    async findRecommendedMatches(
        ...
        const candidates = await this.prisma.user.findMany({
            where: {
                ...
                deletedAt: null,
                birthdate: {
                    gte: ageMax ? new Date(`${new Date().getFullYear() - ageMax}-01-01`) : undefined,
                    lte: ageMin ? new Date(`${new Date().getFullYear() - ageMin}-12-31`) : undefined,
                  },
                  OR: [
                    {
                      interests: {
        ...
        // 벡터값 기반 소프트매치 + 가공 
        const scored = candidates.filter((user)=> {
            const userVector = user.vibeVector as number[]
            // 벡터값 존재하고, 길이가 동일한 유저만 필터링함
            return Array.isArray(userVector) && userVector.length === myVector.length;
        })
        .map((user) => {
            const userVector = user.vibeVector as number[];
            // 코사인 유사도 계산해서 저장 
            ...
            const hasCommonInterest = userInterestIds.some((id) => myKeywordIds.includes(id));
            if (hasCommonInterest) {
                reasons.push('공통 관심사 존재');
            }
            
            // 공통 성향 존재
            const userPersonalityIds = user.personalities.map((p) => p.personalityId);
            const hasCommonPersonality = userPersonalityIds.some((id) => myKeywordIds.includes(id));
